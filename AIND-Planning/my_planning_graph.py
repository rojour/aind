...
class PgNode_s(PgNode):
    ...
    def __init__(self, symbol: str, is_pos: bool):
        ...
        self.symbol = symbol
        self.is_pos = is_pos
        #This code merge is_pos and symbol, and transform it in the same format
...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a):
        ...
        a1_cancels_a2 = [eff in node_a2.action.effect_rem for eff in node_a1.action.effect_add]
        a2_cancels_a1 = [eff in node_a2.action.effect_rem for eff in node_a1.action.effect_add]
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a):
        ...
        a1_add_excl_a2 = [eff in node_a1.action.effect_add for eff in node_a2.action.precond_neg]
        a1_rem_excl_a2 = [eff in node_a1.action.effect_rem for eff in node_a2.action.precond_pos]
        a2_add_excl_a1 = [eff in node_a2.action.effect_add for eff in node_a1.action.precond_neg]
        a2_rem_excl_a1 = [eff in node_a2.action.effect_rem for eff in node_a1.action.precond_pos]

        return any(a1_add_excl_a2) or any(a1_rem_excl_a2) or any(a2_add_excl_a1) or any(a2_rem_excl_a1)
    ...
    def h_levelsum(self):
        ...
        for goal in self.problem.goal:
            level_sum += self.determine_level_cost(goal)
        return level_sum

        #create a function to calculate individual  costs (not to run too many loops together)
    def determine_level_cost(self, goal):
